...
console.log(`Trigger:      Daily at ${args.at} + logon catch-up (local)`);
...
function installWindows(at) {
  ...
  // Daily @ HH:MM + logon catch-up (3 min after login if today's report missing).
  const psScript = `
...
$actionLogon = New-ScheduledTaskAction \`
    -Execute '${nodeExe}' \`
    -Argument '"${wrapperPath}" --logon' \`
    -WorkingDirectory '${projectRoot}'
...
Register-ScheduledTask \`
    ...
    -Settings $settings \`
    -Principal $principal \`
    -Description "Generate daily AI/finance/politics/trading digest at ${at}" \`
    -Force | Out-Null

Register-ScheduledTask \`
    -TaskName "DailyBriefLogon" \`
    -Action $actionLogon \`
    -Trigger $triggerLogon \`
    -Settings $settings \`
    -Principal $principal \`
    -Description "Catch-up daily digest after login (if after ${at} and not yet generated)" \`
    -Force | Out-Null

Write-Host "[OK] Task 'DailyBrief' registered (daily ${at}, StartWhenAvailable)"
Write-Host "[OK] Task 'DailyBriefLogon' registered (3 min after login, catch-up)"
...
console.log(`Logon catch-up:    node "${wrapperPath}" --logon`);
if (args.global) {
